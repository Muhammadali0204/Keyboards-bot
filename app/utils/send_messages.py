...
from app.models.models import MessageButton, Button
from app.utils.others import input_media_type, make_special_text
from app.keyboards.reply import admin_reply_keyboards, reply_keyboards
...
async def send_admin_messages(msg: Message, state: FSMContext, parent_button: Button):
    ...
    if parent_button:
        await msg.answer(f"{msg.text} 🔽", reply_markup=keyboard)
        await state.set_data({"id": parent_button.id})

    if (
        msg.text.replace("🟢", "").replace("🔴", "").strip()
        == "⚡️Taklif havolasini olish⚡️"
    ):
        await msg.answer(
            (
                f"<b>Taklif havolasini https://t.me/{BOT_USERNAME}?start="
                f"{msg.from_user.id} ko'rinishida qo'ymoqchi bo'lsangiz<blockquote>"
                "--link--</blockquote>deb yozing.\n\nAgar biror so'z ostiga link"
                " qo'yilgan holda ko'rinishi kerak bo'lsa <blockquote>--biror matn--"
                "</blockquote> ko'rinishida yuboring!</b>"
            )
        )

        for message in messages:
            if message.message.get("text", None) is not None:
                message.message["text"] = make_special_text(
                    message.message["text"], msg.from_user.id
                )
            elif message.message.get("caption", None) is not None:
                message.message["caption"] = make_special_text(
                    message.message["caption"], msg.from_user.id
                )

    if messages == []:
        await msg.answer("<b>Hech qanday xabar mavjud emas ❌</b>")
        return
    ...
    while i < len(messages):
        ...
        if message.media_group_id is not None:
            ...
            media_message = await msg.answer_media_group(group_messages)
...
async def send_messages(msg: Message, state: FSMContext, parent_button: Button):
    ...
    while i < len(messages):
        ...
        if message.media_group_id is not None:
            ...
            await msg.answer_media_group(group_messages)
        else:
            handlers = {
                MessageType.TEXT: msg.answer,
                MessageType.PHOTO: msg.answer_photo,
                ...
                try:
                    if msg.content_type == MessageType.TEXT:
                        await bot.send_message(
                            user, text=msg.html_text, reply_markup=keyboard
                        )
                        ...
                        await bot.send_sticker(
                            user, sticker=msg.sticker.file_id, reply_markup=keyboard
                        )
                    ...
                    pass
                if (idx + 1) % 100 == 0:
                    await oraliq_xabar(idx, n, bot, admin)
            else:
        ...
        for msg in msgs:
            if msg.content_type == MessageType.VIDEO:
                input_messages.append(
                    InputMediaVideo(media=msg.video.file_id, caption=msg.html_text)
                )
            elif msg.content_type == MessageType.PHOTO:
                input_messages.append(
                    InputMediaPhoto(media=msg.photo[-1].file_id, caption=msg.html_text)
                )
            elif msg.content_type == MessageType.DOCUMENT:
                input_messages.append(
                    InputMediaDocument(
                        media=msg.document.file_id, caption=msg.html_text
                    )
                )
            elif msg.content_type == MessageType.ANIMATION:
                input_messages.append(
                    InputMediaAnimation(
                        media=msg.animation.file_id, caption=msg.html_text
                    )
                )
            elif msg.content_type == MessageType.AUDIO:
                input_messages.append(
                    InputMediaAudio(media=msg.audio.file_id, caption=msg.html_text)
                )

        for idx, user in enumerate(users):
            if not stop_event.is_set():
                try:
                    await bot.send_media_group(user, media=input_messages)
                    await asyncio.sleep(0.04)
                except TelegramAPIError:
                    pass
                if (idx + 1) % 100 == 0:
