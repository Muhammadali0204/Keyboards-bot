...
async def buttons_key(button: Button = None):
    ...
    if button:
        keyboard_list.append(
            [KeyboardButton(text="◀️ Ortga"), KeyboardButton(text="🏠 Bosh menu")]
        )

    top_childs = await Button.filter(
        parent=button, status=ButtonStatus.TOP_ACTIVE
    ).all()
    top_childs.reverse()
    childs = await Button.filter(
        parent=button, status__in=[ButtonStatus.ACTIVE, ButtonStatus.DEACTIVE]
    ).all()

    for i in range(0, len(top_childs) - 1, 2):
        keyboard_list.append(
            [set_button_name(top_childs[i]), set_button_name(top_childs[i + 1])]
        )
    ...
    for i in range(0, len(childs) - 1, 2):
        keyboard_list.append(
            [set_button_name(childs[i]), set_button_name(childs[i + 1])]
        )
    if len(childs) % 2 == 1:
