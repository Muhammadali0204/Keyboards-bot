...
async def send_message_users(msg: Message, state: FSMContext):
    ...
    if task and not task.done():
        await msg.answer(
            (
                ...
                "<i>Bekor qilish uchun <code>Xabar yuborishni to'xtatish ❌</code>"
                " tugmasini bosing !</i></b>"
            )
        )
        return
    ...
    await msg.answer(
        (
            ...
            f"<i>❗️Quyidagi xabar turlarini yuborishingiz mumkin :\n{msg_types}</i>"
        ),
...
async def get_sending_message(msg: Message, state: FSMContext):
    if msg.content_type in MessageType:
        if msg.media_group_id:
            ...
            if (
                msg.message_id
                == temp_data[f"{msg.from_user.id}:messages"][-1].message_id
            ):
                await msg.reply(
                    (
                        "<b>Ushbu xabar barcha foydalanuvchilarga yuborilsinmi ?\n\n"
                        "<i>*Bunday xabar turiga inline tugma qo'sha olmaysiz ❌\n\n"
                        "Istalgan vaqtda <code>Xabar yuborishni to'xtatish ❌</code>"
                        " tugmasi orqali xabar yuborishni to'xtatishingiz "
                        "mumkin</i></b>"
                    ),
            ...
            await msg.reply(
                (
                    "<b>Ushbu xabar barcha foydalanuvchilarga yuborilsinmi ?\n\n"
                    "<i>Istalgan vaqtda <code>Xabar yuborishni to'xtatish ❌</code>"
                    " tugmasi orqali xabar yuborishni to'xtatishingiz mumkin</i></b>"
                ),
...
async def add_inline_button(call: CallbackQuery, state: FSMContext):
    ...
    if temp_data.get(f"{call.from_user.id}:inlines", None) is not None:
        if len(temp_data[f"{call.from_user.id}:inlines"]) < 10:
            ...
            await call.answer(
                "Tugmalar soni : 10 ta, bundan ortiq tugma qo'sha olamysiz !", True
            )
    else:
...
async def get_url(msg: Message, state: FSMContext):
    ...
    if msg.text.startswith("https://"):
        temp_data[f"{msg.from_user.id}:inlines"].append(
            {"name": temp_data[f"{msg.from_user.id}:inline_name"], "url": msg.text}
        )
        await msg.answer("<b>Tugma yaratildi ✅</b>")
...
async def remove_inline_button(call: CallbackQuery):
    ...
    if temp_data.get(f"{call.from_user.id}:inlines", None):
        ...
        await call.message.edit_reply_markup(
            reply_markup=admin_inline_keyboards.send_message_keyboard(
                temp_data[f"{call.from_user.id}:inlines"]
            ),
        )
    else:
        await call.answer("Kutilmagan xatolik !", True)
...
async def start_sending_message(call: CallbackQuery):
    ...
    await call.answer("Xabar yuborilishi boshlandi !", True)
    ...
    await bot.pin_chat_message(call.message.chat.id, message_id=call.message.message_id)
...
async def stop_sending(msg: Message):
    ...
    if task and not task.done():
        ...
        await task
        task = None
        await msg.answer("<b>Yuborilayotgan xabar to'xtatildi !</b>")
    else:
        if task and task.done():
            task = None
        await msg.answer("<b>Yuborilayotgan xabar mavjud emas !</b>")
