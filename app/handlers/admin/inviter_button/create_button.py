...
from app.utils.enums import InviterBtnType, ButtonStatus
...
@router.message(
    InviterStates.get_inviter_btn_name_lim, F.content_type == ContentType.TEXT
)
...
@router.message(
    InviterStates.get_inviter_btn_limit_lim, F.content_type == ContentType.TEXT
)
    ...
    if msg.text.isdigit() and 0 < int(msg.text) < 51:
        ...
        try:
            await InviterButton.create(
                button=btn, type=InviterBtnType.LIMITED, limit=int(msg.text)
            )
            await msg.answer(
            ...
            await Button.create(
                name="🧮Takliflarim soni", parent=btn, status=ButtonStatus.TOP_ACTIVE
            )
            await Button.create(
...
@router.message(
    InviterStates.get_inviter_btn_name_rat, F.content_type == ContentType.TEXT
)
async def get_btn_name(msg: Message, state: FSMContext):
    if len(msg.text) <= 40:
        await state.update_data({"btn_name": msg.text})
...
@router.message(
    InviterStates.get_inviter_btn_limit_rat, F.content_type == ContentType.TEXT
)
async def get_limit(msg: Message, state: FSMContext):
    if msg.text.isdigit() and 0 < int(msg.text) < 51:
        btn_name = (await state.get_data())["btn_name"]
        ...
        try:
            await InviterButton.create(
                button=btn, type=InviterBtnType.RATING, limit=int(msg.text)
            )
            await msg.answer(
            ...
            await Button.create(
                name="Reyting 🏆", parent=btn, status=ButtonStatus.TOP_ACTIVE
            )
            await Button.create(
                name="⚡️Taklif havolasini olish⚡️",
                parent=btn,
