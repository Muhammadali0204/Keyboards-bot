...
from app.models.models import MessageButton, Button, InlineButtonMessage
...
async def add_inline_button(call: CallbackQuery, state: FSMContext):
    ...
    if message:
        ...
        if count > 30:
            await call.answer("Tugmalar soni 30 ta, boshqa qo'sha olmaysiz !", True)
            return
...
async def get_url(msg: Message, state: FSMContext):
    if msg.text.startswith("https://"):
        ...
        inline_button = await InlineButtonMessage.create(
            name=data.get("name", "Nom topilmadi"), url=msg.text, message=message
        )
        if inline_button:
