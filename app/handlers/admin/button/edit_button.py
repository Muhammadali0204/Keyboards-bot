...
async def delete_button(msg: Message, state: FSMContext):
    ...
    if button_id:
        ...
        await msg.answer(
            "<b>❗️O'chirgan tugmangiz tarkibiga kiruvchi barcha tugmalar va xabarlar "
            "o'chib ketadi.</b>",
...
async def info(call: CallbackQuery):
    ...
    await call.answer(
        f"Tugma nomi : {button.name} | Holati : {get_emojiname(button.status)}",
        show_alert=True,
    )
...
async def change_button_status(call: CallbackQuery):
    ...
    await call.answer(
        f"Tugma holati {get_emojiname(status)}'ga o'zgardi ✅", show_alert=True
    )
