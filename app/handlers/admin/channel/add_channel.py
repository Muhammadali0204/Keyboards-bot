...
async def get_channel_id(msg: Message, state: FSMContext):
    if msg.text.isnumeric():
        ...
        try:
            ...
            await msg.answer(
                (
                    "<b>Bot kanalga qo'shilmagan 😕\n\n<i>Botni kanalga qo'shing va "
                    "tegishli huquqlarni bering  va qayta urining !</i></b>"
                )
            )
            await show_panel(msg, state)
...
@router.message(
    ChannelStates.get_channel_name, F.content_type == types.ContentType.TEXT
)
...
@router.callback_query(
    F.data.startswith("channel_type:"), ChannelStates.get_channel_type
)
    ...
    if channel_type in ChannelType:
        ...
        try:
            ...
            if channel_type == ChannelType.DEFAULT:
                channel_link = await bot.export_chat_invite_link(channel_id)
            ...
            await call.answer(f"Xatolik ❌ : {e}", True)
        except Exception as e:
            await call.answer(f"Xatolik ❌ : {e}", True)
    else:
