...
from app.utils.filters import InviteFilter
from app.keyboards.inline import inline_keyboards
from app.utils.others import send_message_to_admins
from app.models.models import User, InviterButton, Channel, Invite
from app.utils.enums import MEDALS, InviteStatus, ChannelType
...
async def invite_count(msg: Message):
    ...
    await msg.answer(f"<b>🧮 Sizning takliflaringiz soni : <i>{count}</i> ta</b>")
...
@router.message(F.text == "Reyting 🏆", InviteFilter())
async def rating(msg: Message):
    inviter = await InviterButton.first()
    users = (
        await User.annotate(
            invite_count=Count(
                "invites", _filter=Q(invites__status=InviteStatus.INVITE_DONE)
            )
        )
