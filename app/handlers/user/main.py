...
async def start_command(msg: Message, command: CommandObject, state: FSMContext):
    try:
        ...
        if inviter_id and inviter_id.isdigit() and int(inviter_id) != msg.from_user.id:
            ...
            if (
                (inviter_btn)
                and (await User.filter(id=inviter_id).exists())
                and (inviter_btn_btn.status) != ButtonStatus.DEACTIVE
            ):
                try:
                    await Invite.create(
                        user=user,
    ...
    if check_data == []:
        ...
        if user and inviter_btn and (inviter_btn_btn.status) != ButtonStatus.DEACTIVE:
            ...
            if invite:
                ...
                count = await Invite.filter(
                    inviter=inviter, status=InviteStatus.INVITE_DONE
                ).count()
                try:
                    ...
                    if inviter_btn.limit <= count:
                        gift_channels = await Channel.filter(
                            type=ChannelType.GIFT
                        ).all()
                        if len(gift_channels) == 1:
                            await bot.send_message(
                                inviter.id,
                                "<b>Tabriklaymiz 🎉\n\nQuyidagi kanalga qo'shilishingiz mumkin 😊</b>",
                                reply_markup=inline_keyboards.gift_channel_list(
                                    gift_channels
                                ),
                            )
                            ...
                            await send_message_to_admins(
                                "Foydalanuvchilarga beriladigan sovg'a kanal mavjud emas ❗️"
                            )
                except Exception as e:
                    print(e)
        elif user is None:
            try:
                await User.create(id=call.from_user.id, name=call.from_user.first_name)
            except OperationalError:
                pass
        await state.clear()
    else:
        await call.answer("Barcha kanallarga a'zo bo'ling❗️", True)
        try:
            await call.message.edit_reply_markup(
                reply_markup=inline_keyboards.channel_list(check_data)
            )
...
async def ortga_(msg: Message, state: FSMContext):
    ...
    if button_id:
        button = await Button.filter(
            id=button_id, status__in=[ButtonStatus.ACTIVE, ButtonStatus.TOP_ACTIVE]
        ).first()
        if button:
...
async def admin(msg: Message):
    await msg.answer(
        (
            ...
            "<i>Ushbu turdagi bot orqali siz o'zingiz xohlaganday tarzda bot "
            "tuzilishini shakllantirib olasiz\nYa'ni botdagi tugmalar va xabarlarni "
            "hech qanday botning kodini o'zgartirmagan va adminga murojaat qilmagan "
            "holda o'zgartirishingiz mumkin !\n\n🤖Shunday ko'rinishdagi bot sizga "
            "kerak bo'lsa, murojaat etishingiz mumkin !</i>"
        )
    )
