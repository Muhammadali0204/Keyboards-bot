...
from utils.enums import ChannelType
...
async def add_channel(call : CallbackQuery):
    ...
    await call.message.edit_text(
        text,
        reply_markup=admin_inline_keyboards.channel_keyboard(channel)
    )
...
@router.message(ChannelStates.get_editing_name, F.content_type == types.ContentType.TEXT)
async def get_editing_name(msg : Message, state : FSMContext):
    if 1 < len(msg.text) <= 40:
        ...
        await msg.answer(
            "<b>Kanal nomi tahrirlandi ✅</b>",
            admin_reply_keyboards.admin_panel
        )
        await state.clear()
...
async def delete_channel(call : CallbackQuery):
    ...
    if channel.type == ChannelType.GIFT:
        try :
            ...
            await call.message.answer(f'<b>Taklif qilish linki kanaldan o\'chirilmadi ❌\n{e}</b>')
