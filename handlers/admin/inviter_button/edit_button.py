...
from models.models import Button, InviterButton, Invite
...
async def stop_limited(call : CallbackQuery, state : FSMContext):
    ...
    if inviter:
        ...
        await call.answer(
            'Taklif qilish tugmasi mavjud emas', True
        )
        await call.message.delete()
...
async def delete_inviter_buttons(inviter : InviterButton):
    ...
    await Invite.filter(status__in=[InviteStatus.INVITED, InviteStatus.INVITE_DONE]).update(status=InviteStatus.INVITE_EXPIRED)
