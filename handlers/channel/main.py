...
from utils.filters import GiftChannelFilter
    ...
    if user and inviter:
        ...
        if inviter_btn.status != ButtonStatus.DEACTIVE:
            ...
            if count >= inviter.limit:
                ...
                try:
                    await bot.send_message(user.id, "<b>Kanalimizga qo'shilish so'rovingiz qabul qilindi ✅</b>")
                except :
                    pass
            else:
                await request.decline()
                try:
                    await bot.send_message(user.id, f"<b>Kanalimizga qo'shilish so'rovingiz qabul qilinmadi ❌\n\nKanalimizga qo'shilish uchun {inviter.limit} ta do'stingizni taklif qiling.\n\n<i>🧮 Sizning takliflaringiz soni : {count} ta</i></b>")
                except :
                    pass
    await request.decline()
