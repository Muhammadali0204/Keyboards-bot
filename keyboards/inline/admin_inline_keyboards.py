...
from utils.others import get_emoji, get_emojiname
from models.models import Button, MessageButton, InlineButtonMessage, Channel
...
send_group_message_key = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text='❇️Xabar yuborishni boshlash', callback_data='start_send_message')
        ],
        [
            InlineKeyboardButton(text='❌Bekor qilish', callback_data='cancel_message')
        ]
    ]
)
...
create_inviter_button = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text='❇️ Limitli inviter button yaratish', callback_data='create_limited')
        ],
        [
            InlineKeyboardButton(text='❇️ Reytingli inviter button yaratish', callback_data='create_rating')
        ]
    ]
)
...
def channels_keyboard(channels : List[Channel]):
    ...
    for channel in channels:
        builder.button(text=f'{channel.name} {'🎁' if channel.type == ChannelType.GIFT else '📊'}', callback_data=f'channel:{channel.id}')
